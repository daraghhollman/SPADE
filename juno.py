...
class PdsDownloader(Downloader):

    def __init__(self, urls: list[str], dataDirectory: str) -> None:
        self.urls = urls
        self.dataDirectory = dataDirectory
        super().__init__(urls, dataDirectory)
...
def DownloadData(instrument: str, timeFrame: list[str], dataDirectory: str) -> None:
    ...
    if instrument not in validInstuments:
        raise ValueError(f"Instrument: '{instrument}' is not a valid choice for the Juno spacecraft")
    ...
    match instrument:
        ...
        case "JAD-E":
            ...
            # There are two possible resolutions, we download both resolutions if both exist as to stich them together later.
            pdsUrl = GetPdsUrl("JAD-E")

            binaryPaths = [f"{pdsUrl}{pathAddition}" for pathAddition in
                            tools.PathsFromTimeDifference(timeFrame[0],
                                                          timeFrame[1],
            ...
            labelPaths = [f"{pdsUrl}{pathAddition}" for pathAddition in
                            tools.PathsFromTimeDifference(timeFrame[0],
                                                          timeFrame[1],
            ...
            Downloader = PdsDownloader(labelPaths + binaryPaths, downloadDirectory)
            Downloader.Download()
            ...
            # Better practice long term would be do detect this before downloading / saving the file,
